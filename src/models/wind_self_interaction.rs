...
impl InitialModel for WindShock {
    ...
    fn primitive_at(&self, coordinate: (f64, f64), t: f64) -> AnyPrimitive {
        ...
        if t >= self.flare_time && t < self.flare_time + self.flare_duration {
            let r = coordinate.0;
            let u = self.flare_gamma_beta * (self.flare_time + self.flare_duration - t).powi(4) / self.flare_duration.powi(4);
            let n = self.flare_outflow_rate / (4.0 * PI * r * r * u * LIGHT_SPEED);
            let rho = n * (t - self.flare_time).powi(4) / self.flare_duration.powi(4);
            ...
            let r = coordinate.0;
            let u = self.flare_gamma_beta * (self.flare_time + 60.0 + self.flare_duration - t).powi(4) / self.flare_duration.powi(4);
            let n = self.flare_outflow_rate / (4.0 * PI * r * r * u * LIGHT_SPEED);
            let rho = 5.0 * n * (t - self.flare_time - 60.0).powi(4) / self.flare_duration.powi(4);
            let p = rho * UNIFORM_TEMPERATURE;
